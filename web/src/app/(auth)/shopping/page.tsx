                ...
                <div key={item.productId} className="rounded-2xl border border-border bg-surface p-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      ...
                      aria-label={alreadyAdded ? `${product.name} už je na seznamu` : `Přidat ${product.name}: ${formatStockQuantity(purchaseQuantity, item.unit, product.package_quantity, product.package_unit)}`}
                    >
                      {alreadyAdded ? <Check size={17} aria-hidden="true" /> : <Plus size={17} aria-hidden="true" />}
                      {alreadyAdded ? 'Přidáno' : formatStockQuantity(purchaseQuantity, item.unit, product.package_quantity, product.package_unit)}
                    </button>
                  </div>
                </div>
              )
            ...
            {openItems.map((item) => {
              ...
              return (
                <div key={item.id} role="group" aria-label={`Nákupní položka ${item.name}`} className="flex min-h-14 items-center gap-2 px-3 py-2">
                  <button type="button" onClick={() => void toggleItem(item.id, item.checked)} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Označit ${item.name} jako nakoupené`}>
          ...
          <details className="rounded-2xl border border-border bg-surface">
            ...
            <div className="divide-y divide-border border-t border-border">
              {completedItems.slice(0, 20).map((item) => (
                <button key={item.id} type="button" onClick={() => void toggleItem(item.id, item.checked)} className="flex min-h-12 w-full items-center gap-3 px-4 py-3 text-left text-sm text-text-muted line-through hover:bg-surface-muted" aria-label={`Vrátit ${item.name} na nákupní seznam`}>
