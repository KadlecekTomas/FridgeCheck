...
export function EditBatchDetailsAction({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.warning(`${productName} bylo upraveno, ale aktuální data se nepodařilo znovu načíst.`)
      setSubmitting(false)
      setOpen(false)
      return
    }
          ...
          <div role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} className="w-full max-w-md rounded-2xl border border-border bg-surface p-5 shadow-xl sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 id={titleId} className="mt-1 text-xl font-bold tracking-[-0.02em] text-text">Upravit balení</h2>
              </div>
            ...
            <form onSubmit={submit} className="mt-5 space-y-4">
              <div>
                ...
                <select id={storageIdField} ref={storageRef} className="input-field" value={storageId} onChange={(event) => setStorageId(event.target.value)} required>
                  {storageUnits.map((storage) => <option key={storage.id} value={storage.id}>{storage.name}</option>)}
                </select>
              </div>
              <div>
              ...
              {expiryType !== 'unknown' ? (
                <div>
                  ...
                  <input id={expiryDateId} className="input-field" type="date" value={expiryDate} onChange={(event) => setExpiryDate(event.target.value)} required />
                </div>
              ) : null}
              <div>
