...
export function EditProductMetadataAction({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.warning(`${normalizedName} bylo uloženo, ale aktuální data se nepodařilo znovu načíst.`)
      setSubmitting(false)
      setOpen(false)
      return
    }
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 id={titleId} className="mt-1 text-xl font-bold tracking-[-0.02em] text-text">Upravit údaje produktu</h2>
              </div>
            ...
            <form onSubmit={submit} className="mt-5 space-y-4">
              ...
              <div>
                ...
                <input id={brandId} className="input-field" value={brand} onChange={(event) => setBrand(event.target.value)} maxLength={200} placeholder="volitelné" />
              </div>
              <div>
                <label htmlFor={eanId} className="field-label">EAN</label>
                <input id={eanId} className="input-field" value={ean} onChange={(event) => setEan(event.target.value)} inputMode="numeric" autoComplete="off" placeholder="8 až 14 číslic" />
              </div>
              <div>
                <label htmlFor={categoryId} className="field-label">Kategorie</label>
                <input id={categoryId} className="input-field" value={category} onChange={(event) => setCategory(event.target.value)} maxLength={200} placeholder="volitelné" />
              </div>
