...
begin
  ...
  select id into strict fridge_a
  ...
  values (household_a, 'Mrazák', 'freezer')
  returning id into freezer_a;
  ...
  if public.update_product_metadata(
    product_a,
    '  Bílý jogurt  ',
    '  Nová značka  ',
    '8591234567890',
  ...
  if not exists (
    ...
    where id = product_a
      and name = 'Bílý jogurt'
      and brand = 'Nová značka'
      and ean_code = '8591234567890'
      and category = 'Jogurty'
  ...
  if not exists (
    ...
    from public.inventory_batches
    where id = batch_a
      and storage_unit_id = freezer_a
      and expiry_type = 'best_before'
      and expiry_date = '2026-08-25'
      and quantity = 4
      and status = 'active'
  ) then
  ...
  begin
    perform public.update_inventory_batch_details(
      batch_a,
      freezer_a,
      'best_before',
      '2026-08-25',
  ...
  select id into strict fridge_b
  from public.storage_units
